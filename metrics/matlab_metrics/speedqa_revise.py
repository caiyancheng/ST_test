...
class speedqa_metric(vq_metric):
    ...
    def __init__(self, device=None) -> None:
        ...
        self.log_file = "matlab_log.txt"
        if os.path.exists(self.log_file):
            os.remove(self.log_file)
        cmd = (
            f'matlab -nosplash -nodesktop -wait '
            f'-sd metrics/matlab_metrics '
            f'-logfile {self.log_file} '
            f'-r "disp(pwd);worker_SPEEDQA();quit;"'
        )
        self.proc = Popen(cmd.split(), stdin=PIPE, stdout=PIPE, stderr=PIPE)
        ...
        while True:
            if os.path.exists(self.log_file):
                with open(self.log_file, "r", encoding="utf-8", errors="ignore") as f:
                    lines = f.readlines()
                if any("<start>" in line for line in lines):
                    break
                if time.time() - start_time > timeout:
                    raise RuntimeError("MATLAB worker 启动超时 for SpeedQA（>20s），请检查是否正常运行。")
            time.sleep(0.2)
        # while True:
    ...
    def predict_video_source(self, vid_source, frame_padding="replicate"):
        ...
        for ff in range(N_frames):
            ...
            #
            # if N_frames == 1:
            #     with tempfile.NamedTemporaryFile(mode='w+b', prefix='SPEEDQA' + "-", suffix=".mat") as tf:
            ...
            if N_frames == 1:
                # ✅ 改成 mkstemp，避免 Windows 文件锁
                fd, temp_path = tempfile.mkstemp(prefix="SPEEDQA-", suffix=".mat")
                os.close(fd)  # 关闭文件描述符
                try:
                    frames = {'Yr': Yr, 'Yt': Yt, 'Yr_prev': Yr, 'Yt_prev': Yt, 'type': 'image'}
                    scipy.io.savemat(temp_path, frames)

                    self.proc.stdin.write(bytes(f'c {temp_path}\n', 'utf-8'))
                    self.proc.stdin.flush()
                    res = self.proc.stdout.readline()
                ...
                if ff > 0:
                    fd, temp_path = tempfile.mkstemp(prefix="STRRED-", suffix=".mat")
                    os.close(fd)
                    try:
                        frames = {'Yr': Yr, 'Yt': Yt, 'Yr_prev': Yr_prev, 'Yt_prev': Yt_prev, 'type': 'video'}
                        scipy.io.savemat(temp_path, frames)
