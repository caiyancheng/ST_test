...
class hyperiqa_metric(vq_metric):
    ...
    def __init__(self, device=None):
        ...
        self.metric = create_metric('hyperiqa', device=self.device)
    ...
    def predict_video_source(self, vid_source, frame_padding="replicate"):
        ...
        for ff in range(N_frames):
            ...
            test_score = self.metric(T)
            ref_score = self.metric(R)

            score += -1*(ref_score - test_score) / N_frames
